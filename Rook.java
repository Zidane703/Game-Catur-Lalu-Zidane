	...
	public void generateTakeOpponentPieceMoves(ChessBoard board) {
		...
		for (int i = startRow; i > 0; i--) {
			if (board.get(i-1, startColumn).getChar() == '-') 
				...
				MoveCommand temp = new MoveCommand(startRow, startColumn, i-1, startColumn);
				temp.setTakePieceDiff(board.get(i-1, startColumn).getPieceValue() - pieceValue);
		...
		for (int j = startColumn; j > 0; j--) {
			if (board.get(startRow, j-1).getChar() == '-') 
				...
				MoveCommand temp = new MoveCommand(startRow, startColumn, startRow, j-1);
				temp.setTakePieceDiff(board.get(startColumn, j-1).getPieceValue() - pieceValue);
				takeOpponentPieceMoves.add(temp);
				break;
			}
			else
		...
		for (int j = startColumn; j < 7; j++) {
			if (board.get(startRow, j+1).getChar() == '-') 
				...
				MoveCommand temp = new MoveCommand(startRow, startColumn, startRow, j+1);
				temp.setTakePieceDiff(board.get(startRow, j+1).getPieceValue() - pieceValue);
				takeOpponentPieceMoves.add(temp);
				break;
			}
			else
